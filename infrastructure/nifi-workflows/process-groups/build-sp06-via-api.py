#!/usr/bin/env python3
"""
Script per creare il flusso SP06 via API NiFi (senza template).
SP06 - Validator (Semantic and Compliance Validation)
"""
...
def build_sp06(builder: NiFiFlowBuilder):
    print("🏗️  Costruzione SP06 - Validator...")
    pg = builder.create_process_group("SP06_Validator")
    pg_id = pg["id"]
    ...
    listen_http = builder.create_processor(pg_id, "org.apache.nifi.processors.standard.ListenHTTP", "HTTP_Endpoint", 100, 100, {
        "properties": {"Listening Port": "9096", "Base Path": "sp06"}
    })
    listen_id = listen_http["id"]
    print(f"  ✅ ListenHTTP creato: {listen_id} (porta 9096)")

    invoke_http = builder.create_processor(pg_id, "org.apache.nifi.processors.standard.InvokeHTTP", "Call_SP06_Validator", 300, 100, {
        "properties": {"HTTP Method": "POST", "Remote URL": "http://sp06-validator:5006/validate", "Content-Type": "application/json"},
    ...
    print(f"\n🎉 SP06 costruito con successo! Process Group ID: {pg_id}")
    return pg_id

if __name__ == "__main__":
    print("=" * 60)
    print("NiFi Flow Builder - SP06 Validator")
    print("=" * 60)
    builder = NiFiFlowBuilder(NIFI_URL)
    try:
        sp06_id = build_sp06(builder)
        print("\n" + "=" * 60)
        print("✅ COMPLETATO!")
        print("=" * 60)
        print(f"\nProcess Group SP06 ID: {sp06_id}")
        print("\nEndpoint: POST http://localhost:9096/sp06")
