#!/usr/bin/env python3
"""
Script per creare il flusso SP03 via API NiFi (senza template).
...
def build_sp03(builder: NiFiFlowBuilder):
    """
    Costruisce il flusso SP03 - Procedural Classifier.

    Architettura semplificata:
    - ListenHTTP (porta 9093) -> riceve POST HTTP con dati istanza
    - InvokeHTTP -> chiama microservizio SP03 su porta 5003
    - Output Port "Success" -> risultati OK (auto-terminated)
    """
    print("🏗️  Costruzione SP03 - Procedural Classifier...")

    # 1. Crea Process Group
    pg = builder.create_process_group("SP03_Procedural_Classifier")
    pg_id = pg["id"]
    ...
    print(f"  ✅ ListenHTTP creato: {listen_id} (porta 9093)")

    # 3. Crea InvokeHTTP (chiama microservizio SP03)
    invoke_http = builder.create_processor(
        pg_id,
        "org.apache.nifi.processors.standard.InvokeHTTP",
        "Call_SP03_Microservice",
        x=300, y=100,
        config={
            "properties": {
                "HTTP Method": "POST",
                "Remote URL": "http://sp03-procedural-classifier:5003/classify-procedure",
    ...
    print(f"\n🎉 SP03 costruito con successo! Process Group ID: {pg_id}")
    return pg_id


if __name__ == "__main__":
    print("=" * 60)
    print("NiFi Flow Builder - SP03 Procedural Classifier")
    print("=" * 60)
    print()
    ...
    try:
        sp03_id = build_sp03(builder)

        print("\n" + "=" * 60)
        print("✅ COMPLETATO!")
        print("=" * 60)
        print(f"\nProcess Group SP03 ID: {sp03_id}")
        print("\nAccedi a NiFi UI: http://localhost:8080/nifi")
        print("Vedrai il process group 'SP03_Procedural_Classifier' con:")
        print("  - ListenHTTP su porta 9093 (endpoint: POST http://localhost:9093/sp03)")
        print("  - InvokeHTTP per chiamare microservizio SP03")
        print("  - Output Port 'Success' (auto-terminated)")
        print("\n📮 Test con Postman:")
        print("   POST http://localhost:9093/sp03")
        print("   Body: JSON con dati istanza amministrativa")
